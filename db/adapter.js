...
async function migrateV2() {
  if (db.type === 'postgres') {
    try { await db.exec('ALTER TABLE qr_codes ADD COLUMN IF NOT EXISTS verify_code_hash TEXT DEFAULT \'\''); } catch {}
    try { await db.exec('ALTER TABLE qr_codes ADD COLUMN IF NOT EXISTS file_size INTEGER DEFAULT 0'); } catch {}
  } else {
    try { db.exec('ALTER TABLE qr_codes ADD COLUMN verify_code_hash TEXT DEFAULT ""'); } catch {}
    try { db.exec('ALTER TABLE qr_codes ADD COLUMN file_size INTEGER DEFAULT 0'); } catch {}
  }
}
